...
## change from here
    ...
    #vae.summary()
    vae.train_on_batch(np.array([[0.]*65])) #,  validation_data=np.array([[0]*65])) #, callbacks=[lrs])
    vae.load_weights(model_fn,by_name=True)
    vae.summary()

def get_args():
    p = ArgParser()
    p.add('-c', '--config_file', default='nn_config/precip_predict_VAE.yml', is_config_file=True, help='Path to config file.')
    # Data arguments
